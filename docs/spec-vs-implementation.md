...
|---|---|---|
| Bridge pen / eraser, draw-merge, lasso → group / shape as **canvas gestures** | the same grep now finds callers in `ui/MacPaintUI.cpp`: the flats-tool canvas route, and `flatsLassoCommit()` at both `selectPolygon()` commit sites | **built.** Reached as sticky tools from the FLATS TOOLS palette (`ControlsSection::FlatsTools`), which is what `docs/shortcuts.md` §1.1 meant by "the scoped set must be visible". **Keys: bound** (2026-09-13). Besides `K`, `U`, `,` / `.` / `Return` and `⌘⇧K`, `keymaps/default.json` now binds the rest of `docs/shortcuts.md` §1.1's set in Flats scope: `Y` shape fill, `⇧K` group lasso, `⇧U` draw-merge, `⇧B` draw bridge, `⇧V` select edits, `⇧Return` accept all. Keys and palette share `toggleFlatsTool()`; the key-action table is `flatsActionForKeyAction()` (`app/ToolSwitch`). Accept-all is not recordable, like every other Flats action. |
| `FLATS · N FILLS` sub-line, Fills panel | `layerRowSubLine(const Document&, size_t)` (`app/LayerPanel`) | **sub-line built** (2026-09-13): a Flats row reads `FLATS · N FILLS`, the live-fill count of the cached evaluation through `flatsPeekEvaluation()`, and `app/selftest/LayerPanel2a.cpp` now asserts the count instead of its absence. **Not built:** a per-fill list (Fills panel). |
...
**The Flats-scoped set is bound** since 2026-09-13 (§1's first row). It was
data, as expected: the load-time conflict detector already scoped by layer
kind.
...
| 6 — Filter and transform it (`PLAN.md:448`) | **transform built; filters built destructively; the phase's own architecture absent.** Built: the resample kernels the phase names, exact flips and 90° paths, crop / canvas size / image size, free and numeric transform, **straighten** and **perspective correction** (both in `app/CropTool` — four free corners through `transformFromQuad()`, `:416`), the gradient tool, the paint bucket, the Filter menu (Highpass, Local Contrast and Lens Correction reachable since 2026-09-13), the **Warp** mode of Free Transform (D23, `app/WarpMesh`) and **Fill / Stroke** with colour, pattern or gradient (D26, `ops/Fill`). **Not built, heaviest first:** (1) **No filter is live.** The phase opens with ROI propagation and a tile cache keyed on the ops below; neither exists. `core/OpStack.hpp:41` says only `OpClass::PointA` has real ops — `SpatialB` is a tag nothing constructs outside a fixture — so every spatial op writes texels through `app/PixelOpBridge` and cannot be re-edited. That fails PRD D4 (P1, "live and re-editable") and D12's "spatial, not point ops" framing. (2) ~~Dust & scratches (D11) and shadows/highlights (D12)~~ — built destructively on 2026-09-13: Filter > Dust & Scratches…, Image > Adjustments > Shadows/Highlights… (`ops/Filters`). (3) The D23/D26 edges: ~~Stroke refuses without a selection, Fill and Stroke have no live preview~~ — built 2026-09-13: with nothing selected Stroke traces the active layer's content edge (where alpha crosses 50%) and refuses an empty layer by name, and both dialogs preview live; Warp still refuses Pigment, masked and layer-set targets. (4) ~~Lens blur and radial / spin / zoom blur (P2)~~ — built destructively on 2026-09-13: Filter > Radial Blur… (Spin or Zoom, `ops/RadialBlur`, with centre and amount handles on the canvas while the dialog is open, `app/RadialBlurHandles`) and Filter > Lens Blur… (`ops/LensBlur`). Puppet warp is deferred by the phase itself. |
| 7 — Select and paste (`PLAN.md:480`) | **built.** `core/Channels` implements all three of PRD E11–E13 in one file — named alpha channels persisted in `.npaint`, selection ↔ channel both ways, and quick mask. Since 2026-09-13 all of it is reachable: Save Selection as Channel… and Load Channel as Selection… menu entries, Edit in Quick Mask Mode (Q, painted through `brush/QuickMaskPaint`), and a CHANNELS dock panel with rename and delete (`app/ChannelsPanel`). **PRD M9** landed the same day: Paste Into (⇧⌥⌘V), Paste as New Document (`app/PasteCommands`) and ⌥-drag duplicate (a floating copy with a selection, a duplicated layer without). |
| 8 — Repair it (`PLAN.md:505`) | **mostly built as of 2026-09-09**, and this table omitted the phase entirely until that morning. `LayerKind::Strokes` now holds dab records with a spatial index over their bounds, checkpoint tiles and the samples-only-from-below rule (`core/StrokesContent`, `brush/StrokesLayer`, `io/StrokesSerial`); it rasterises per PRD C11, round-trips as `np:dabs`, and erases by deleting the records a stroke covers (PRD F11). **Heal** is `Tool::Heal` over a gradient-domain solve (`ops/Poisson`, `brush/Heal`); **diffusion inpaint** is `ops/Inpaint` (Telea) through `app/PixelOpBridge`. Clone remains *aligned only* — `brush/CloneStamp.hpp:218` records that toggle as a deliberate omission, so it is not an absence. The **class-C form of clone and heal** landed on 2026-09-10: `StrokeRoute::StrokesRecord` (one route for both tools, `app/StrokeSession` §1d argues the call) appends a `DabRecord` to a Strokes layer instead of writing texels, and `brush/StrokesLayer` re-evaluates it against the composite beneath — so a repair tracks a regrade under it, which is PRD D6 and `docs/operations.md:341`'s class C. A recorded heal is `DabColorSource::BelowHealed`, a solve whose source is the below-composite at the offset and whose Dirichlet rim is the below-composite straight down (`brush/StrokesLayer` §1b). That closed the reachability hole this row used to describe from the other end: `Layer > New Strokes Layer` made a real layer that nothing in the build could put a mark in, and PRD F11's erase worked correctly on records nothing could make. **PatchMatch** (D7's textured half) landed 2026-09-13 as Edit > Content-Aware Fill… (`ops/PatchMatch`): **destructive and seeded by the user's decision**, not the cached class-D op the phase names — Recompute draws a new seed and the seed is recorded, so a replay is exact. **Not built:** a recorded PAINT stroke (`DabColorSource::Ink`), which is deferred by name — `app/StrokeSession` §1d states the three questions it is blocked on. |
| 9 — Tile it (`PLAN.md:510`) | **partly built as of 2026-09-09.** PRD D8's first, second and fourth pieces are in: **lighting-gradient removal** (divide by a heavily blurred copy and re-centre the mean, `ops/Filters`), **offset with wrap** (an addressing change that does no filtering), and the **3×3 repeat preview** (`app/TilePreview`, a View-menu toggle drawing nine quads through `ui/CanvasQuad`, golden view `tile_preview`). **Seam heal** landed 2026-09-13 as Filter > Seam Heal… (`ops/SeamHeal`): offset by half, PatchMatch over both seam bands, offset back — destructive and seeded, on the same engine as Phase 8's Content-Aware Fill. Every PRD D8 piece is now in; what differs from the phase text is only that PatchMatch is not a cached class-D op. All three built ops are recordable since the gap-closing wave (Phase 19 row). |
| 10 — Paint on it (`PLAN.md:517`) | **built.** Brush and dynamics, deposit on RGB and Pigment, the eraser per layer kind (ADR-0007), pencil, dodge and burn, and smudge on RGB and — since `16ead20` — on Pigment (`brush/PigmentSmudge`, a mass-weighted mean of latents). Dodge and burn are destructive, which PRD D13 now accepts (owner decision, 2026-09-12). The Media deposit waits on Phase 11. This pass checked the tools and routes only — not the latency path or the `⌃⌥`-drag size gesture. |
...
* **`docs/operations.md`**: polar↔rectangular remap is still marked "future work"
  (`:191`). Radial, spin, zoom and lens blur (P2, `:132–134`) landed 2026-09-13 as
  `ops/RadialBlur` and `ops/LensBlur`, destructive through the filter bridge like
  the rest (§3, Phase 6). That document's P0/P1/P2 tables do not consistently
...
* ~~There is no `--version` flag~~ — landed 2026-09-13: `--version` / `-v` print
  `naturalPaint <version> (<hash>[-dirty])` from a build-time generated header, and
  `--help` / `-h` exist too.
* ~~**Menu entries for the headless engines**~~ — landed 2026-09-13:
  Highpass, Local Contrast and Lens Correction in the Filter menu, Edit >
  Define Pattern…, and pattern fills through Edit > Fill….
...
**2026-09-13** (on `integrate/p2` at `45e809c`, after the P2 wave):

* **Closed, each re-checked in the merged tree:** dust & scratches (D11),
  shadows/highlights (D12), radial/spin/zoom and lens blur, PatchMatch as
  Content-Aware Fill (D7) and seam heal (D8), `--version`, the six Flats keys
  and the `FLATS · N FILLS` sub-line. All of the new filters are destructive
  and recordable; PatchMatch and seam heal are seeded, not cached class-D ops,
  by decision.
* **Split view (PRD A5) already existed.** The wave added what it lacked: the
  companion pane keeps its own zoom and pan, a click focuses a pane, and
