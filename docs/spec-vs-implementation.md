...
|---|---|---|
| Bridge pen / eraser, draw-merge, lasso → group / shape as **canvas gestures** | `grep -rn 'flatsBridgeStroke\|flatsDrawMerge\|flatsGroupFromPath\|flatsShapeFromPath' src/ui src/main.cpp src/app` finds no caller outside `flats/` and the selftest | **model functions built and tested; not reachable from the canvas.** The seams the plan names are the lasso commit in `ui/MacPaintUI.cpp` and the Pencil/Eraser stroke route on a Flats layer. This is the same "built headless, no UI caller" shape as §3's path consumers. |
| `FLATS · N FILLS` sub-line, Fills panel | `app/LayerPanel.cpp:123` returns no sub-line for Flats; no `FillsPanel` symbol anywhere in `src/` | not built. The comment at `ui/MacPaintUI.cpp:2117` still says "a Flats layer has no fills", which was true before the port and is not now — the model has fills, the row just does not count them. |
| Expand to layers (N9), PSD group export | no symbol | not built |
...
(`1d6db71`) with Phase 14 behind it.
...
* **`ui/MacPaintUI.cpp:2117`'s comment** ("a Flats layer has no fills") is a
  ...
  kind of line a future survey would read and believe.
