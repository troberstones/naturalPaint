...
| 13 — Paths | **partly built.** `core/Path`, `core/PathFlatten`, `core/PathRaster`, `core/PathStroke`, `core/VectorShape`, `LayerKind::Vector`, `io/PathSerial`, `io/SvgImport` and `app/PenTool` all exist; Pen/Curve have a canvas gesture, an on-canvas overlay, a **MODE segment** (Shape / Component, with a SELECTED readout) and a **drawn gnomon** — bounds box, four scale corners, two axis arrows and the rotate ring, all read from `gnomonHandlePositions()` so the drawn geometry and the hit geometry cannot disagree. `pathEditBegin()` forwards `gnomonReachPx` for that reason, asserted in `app/selftest/PenTool.cpp` §5b with its own sabotage proof — the first version of that assertion tested `hitTestPath()` directly, did not exercise the forwarding, and stayed green under the exact defect. Vector and Text layers draw their geometry into the LAYERS thumbnail (`app/LayerThumbnail` §5), rasterised at 24x24 with no cache — `layerHoldsPixels()` was deliberately NOT touched, since flipping it enables layers the composite walk still skips past and two sites would then deref a disengaged `optional`. `app/PathConsumers` supplies the three PRD J consumers headless — path-to-selection (reusing `combineSelections()` and asserted against `selectPolygon()` texel for texel), fill path, and stroke path through `brush/StrokePath`; **they have no UI caller yet**, which is the same gap `pathEditSetSelectMode()` was in. **Not built, by name:** the PATHS dock tab, and the gestures that would invoke those three. |
| 14 — Text | **built.** `text/Shaper` + `text/CoreTextShaper.mm` (+ a stub elsewhere), `core/TextContent`, a live `LayerKind::Text` layer, `np:text` in `io/NpaintFile`, `app/TextTool`, the canvas gesture and overlay, and an options row with FONT / SIZE / B / I / ALIGN / COLOR. `Tool::Text` is marked built and `LayerCommand::NewTextLayer` makes an empty one from the LAYERS panel. **Not built, by name:** selection ranges — the caret is a single byte offset, so there is no shift-click, no double-click-a-word and no styled run. SVG `<text>` imports through `io/SvgImport` as a Text layer when the element reduces to one styled run under a translate-plus-uniform-scale, and as glyph outlines otherwise — with the fallback named per element in the report, never silent. SVG `<text>` imports through `io/SvgImport` as a Text layer when the element reduces to one styled run under a translate-plus-uniform-scale, and as glyph outlines otherwise — the fallback named per element in the report, never silent; a file with text opens as an alternating Vector/Text stack so painting order survives. Text on a path, vertical text and rich-text runs are non-goals (PRD.md:102). |
