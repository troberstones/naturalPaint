...
#
# The master is a 1024x1024 PNG exported from icons/np_icon.kra. Everything
# under icons/macos, icons/windows/*.ico, icons/linux/hicolor and icons/ios is
# GENERATED by this script and committed, so a build never needs ImageMagick
# or iconutil -- rerun this and commit the result after editing the artwork.
#
# ImageMagick (`magick`) is used when present; otherwise macOS's own `sips`
# does the resizing and a few lines of Python pack the Windows .ico (an .ico
# may hold PNG-compressed entries, which every Windows since Vista reads), so
# a stock Mac produces every output. Folded from the two scripts the icon was
# first generated with (makeIcon.sh, makeIcon2.sh): the second was the first
# plus a sips fallback that silently skipped the .ico.
set -euo pipefail
...
# Only the generated FILES are cleared -- never $HERE or a whole platform
# directory: $HERE holds the master and the Krita source, and macos/,
# windows/ and linux/ each hold a hand-written file beside the generated one
# (Info.plist.in, naturalPaint.rc.in, the .desktop entry and its installer).
