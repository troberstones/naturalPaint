// color/LutBake per-op kernel: Exposure (PLAN.md Phase 3 step 4). WGSL port
// of ops/PointOps.cpp's applyExposure() -- `output = input * exp2(stops)`,
// deliberately linear-light, not the shaper domain (docs/operations.md:
// "a pure multiply in linear"). See lut_op_levels.wgsl's header comment for
// the shared decode/op/encode/clamp shape every kernel here follows.
//#include "include/shaper.wgsl"

@group(0) @binding(0) var<uniform> P : vec4<f32>;  // x = stops, yzw unused
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  let linearOut = linearIn * exp2(P.x);
