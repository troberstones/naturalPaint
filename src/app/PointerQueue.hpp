...
//
// **Timestamps are used to associate events with each other, and, since Wave
// 2, in one further place: `brush/Stabiliser`'s weighted-average mode, which
// `strokeSampleFromPointer()` now carries `PointerSample::timestamp` into
...
// distance is measured along, never the spacing rule itself. Before Wave 2
// nothing downstream of `takeForStroke()` read a sample's timestamp at all.
// The UI sequence numbers of section 3 are not time at all -- they are
// positions in ImGui's input stream.
