...
// every commit function below calls exactly one `core::RegionOps` mutator
// and returns its `LayerOpResult` for the caller to fold through
// `recordLayerEdit()` -- there is no per-frame commit anywhere in this file,
// only a per-frame *preview* rectangle a caller may choose to draw.
//
...
// Ends a `Defining` drag: `core::addRegion()` on `regionDefineRect()`'s
// rectangle. Selects the new region on success. Resets `session` to `Idle`
// either way -- a refused (empty) drag is "a click without a drag creates
// nothing", not a drag left open for another try.
...
// Ends a `Moving` drag: `core::moveRegion()` to `regionMoveOrigin()`'s
// origin. Resets `session.gesture` to `Idle` (selection is kept).
...
// Ends a `Resizing` drag: `core::resizeRegion()` to `regionResizeRect()`'s
// rectangle. Refused, by name (from `resizeRegion()`), if the corner was
// dragged onto its fixed opposite on both axes -- the zero-size case.
// Resets `session.gesture` to `Idle` either way.
LayerOpResult regionCommitResize(RegionSession& session, Document& doc, float curX, float curY);

// Deletes the selected region, if any. `LayerOpResult::ok == false` with no
// error text (not a refusal to surface) when nothing is selected -- a
// Delete keypress with no region selected is not a mistake to report, it is
// simply not this tool's business right now.
