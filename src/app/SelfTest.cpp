...
bool runNpaintFormatTest() {
  ...
  //     -> flatten's sum of a single contribution (exact)
  ...
  // the bound rather than waved away: relative error <= 2^-11 + 3*2^-24 =
  // 4.8830e-4 + 1.79e-7 = 4.8848e-4. The fixture's composite values are all
  ...
  if (kOiioBuild) {
    ...
    check(saved.warnings.empty(), "save: and reports nothing approximate about the result");
    ...
    if (loaded.document.layers.size() == 3) {
      ...
      check(l0.visible == true && l1.visible == false && l2.visible == true,
