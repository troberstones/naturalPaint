...
bool runPigmentLayerTest() {
  ...
  // --- 10. The `.npaint` round trip, Pigment and RGB in one document -----
  {
    ...
    if (kOiioBuild && saved.ok) {
      ...
      // The op stack has no carrier, and that is said rather than dropped.
      ...
      check(gradedSave.ok && gradedSave.warnings.size() == 1 &&
                contains(gradedSave.warnings[0], "op stack") &&
                contains(gradedSave.warnings[0], "np:ops"),
            "npaint: a layer's op stack is not written -- there is no working blob carrier -- "
            "and the save says so by name rather than dropping it quietly (PRD I11)");
    }
    std::remove(kPath);
