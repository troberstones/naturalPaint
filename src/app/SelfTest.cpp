...
bool runNpaintFormatTest() {
  ...
  if (kOiioBuild) {
    ...
    // **Changed by PLAN.md Phase 5 step 1.** This used to assert
    // `saved.warnings.empty()`. It cannot any more, and the reason is the
    // point rather than an inconvenience: two of this fixture's layers carry
    // blend names ("multiply", "screen") that Phase 5 step 2's `core/Blend`
    // will implement and this build does not, so part 0 -- which is
    // regenerated on every save (PRD I12) and is now a real composite rather
    // than a sum -- is an approximation of what those layers mean. The save
    // still goes ahead, because refusing would make a PRD I10-preserved blend
    // name the thing that stops the document being saved (core/Composite.hpp
    // argues that at length); it just says so. Asserting the *content* of the
    // warnings rather than merely their count is what makes "never silently"
    // checkable.
    {
      bool warnedMultiply = false, warnedScreen = false;
      for (const std::string& w : saved.warnings) {
        if (contains(w, "\"multiply\"") && contains(w, "Line pass")) warnedMultiply = true;
        if (contains(w, "\"screen\"")) warnedScreen = true;
      }
      check(saved.warnings.size() == 2 && warnedMultiply && warnedScreen,
            "save: the two layers whose blend this build cannot composite are named, with "
            "their blend, rather than silently composited as `over`");
    }
    ...
    if (loaded.document.layers.size() == 3) {
      ...
      check(l0.blend == "multiply" && l1.blend == "normal" && l2.blend == "screen",
...
bool runLayerStackTest() {
  ...
  {
    check(blendIsImplemented(kDefaultBlendName),
          "blend: \"normal\" is the one blend this build implements");
    check(!blendIsImplemented("multiply") && !blendIsImplemented("Mix") &&
              !blendIsImplemented(""),
          "blend: everything else -- including a future build's `Mix` and an empty string -- "
          "is reported unimplemented rather than assumed to be `over`");
