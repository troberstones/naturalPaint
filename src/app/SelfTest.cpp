...
bool runLayerEditorTest() {
  ...
  // range is well defined to cast.
  {
    ...
    // 13 since PLAN.md Phase 5 step 12 added `CaptureComp`, which is the one
    // comp gesture that carries no value (app/LayerEditor.hpp says why the
    // other four stay in the COMPS panel).
    check(all.size() == 13, "the menu walks all 13 commands");
...
bool runControlsLayoutTest() {
  ...
  // 10 since PLAN.md Phase 5 step 12 added COMPS, a third `Document` section.
  check(sections.size() == 10, "every section has exactly one spec (10)");
  check(newOrder.size() == kOldOrder.size() + 1,
        "the same sections, reordered, plus COMPS -- none was dropped");
  {
