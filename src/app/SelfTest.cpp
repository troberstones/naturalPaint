...
bool runLayerEditorTest() {
  ...
  // range is well defined to cast.
  {
    ...
    check(all.size() == 12, "the menu walks all 12 commands");
...
bool runControlsLayoutTest() {
  ...
  check(sections.size() == 9, "every section has exactly one spec (9)");
  check(newOrder.size() == kOldOrder.size(), "the same sections, reordered -- none was dropped");
  {
...

  {
    ...
    check(exact && open == 2, "exactly the document sections start open (LAYERS, HISTORY)");
  }
