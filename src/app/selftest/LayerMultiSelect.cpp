...
bool runLayerMultiSelectTest() {
  ...
  std::printf("  **group**: the MODEL is built (app/selftest/LayerGroup.cpp: LayerKind::Group, "
              ...
              "reached it, and that claim itself turned out to be wrong rather than merely "
              "stale: `ui/MacPaintUI.cpp`'s LAYERS-panel \"Multi-selection\" section and the "
              "`Layer` > Selection menu both already walked `core::allLayerSetCommands()` "
              "generically, from PLAN.md Phase 5 step 11, well before GroupLayers/UngroupLayers "
              "existed -- so the moment the two joined that list, both surfaces offered them "
              "with no UI code change at all. `main.cpp --demo-document --ui-multiselect-demo "
              "\"select:0.1,group\"` proves it through the exact `applyLayerSetCommand()` funnel "
              "a click uses. What task/group-ui actually added was the other half reachability "
              "needs -- FEEDBACK: a group row read no differently from an ordinary one, with no "
              "way to tell a member was INSIDE it, and no way to collapse a group so a stack of "
              "twenty does not defeat the panel's bounded scroll region. app/LayerPanel now has "
              "`layerGroupDepth()`/`layerGroupAncestry()` (nesting-aware, ordered immediate-"
              "parent-first) and `layerHiddenByCollapsedGroup()`; ui/MacPaintUI.cpp draws the "
              "indent, a per-level guide line, and a disclosure triangle with session-only "
              "collapse state. See app/selftest/LayerGroupPanel.cpp for both halves of this "
              "finding, proven rather than asserted. docs/reachability-audit.md's C7 can close.\n");
