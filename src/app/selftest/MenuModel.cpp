...
bool runMenuModelTest() {
  ...
  // 113 -> 115: PRD P2's `RadialBlur` and `LensBlur` (docs/operations.md
  // §2.2), counted off `MenuAction`'s enumerators in this header (116
  // including `Count`), the way every edition of this comment demands.
  check(kMenuActionCount == 115,
        "ids: exactly 115 actions -- the original 41-item extraction plus D1/D2's "
        ...
        "Contrast/Lens Correction, Warp, and Radial/Lens Blur, so an item lost in "
