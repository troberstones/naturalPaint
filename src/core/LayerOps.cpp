...
// The op-index bounds check, the same shape as core/LayerOpRefusal's
// layerOpInRange(). It stays here, in an anonymous namespace and without the
// `layerOp` prefix, because it is the one helper of the six with a single
// caller -- nothing outside this file walks an op stack -- and internal linkage
// is still the right default for a name with one user.
//
// It is separate from the layer bounds check because it names a different list
// -- "op 4 of layer 2" rather than "layer 4" -- and because it is checked
// *after* the layer index, so it can index `doc.layers[index]` safely.
bool opInRange(const Document& doc, size_t index, size_t opIndex, const char* what,
  ...
  *out = layerOpFail(
      std::string(what) + " refused: there is no op at index " + std::to_string(opIndex) +
      " on " + layerOpDescribe(doc, index) + " -- its stack has " + std::to_string(count) +
...
LayerOpResult addLayer(Document& doc, size_t index, Layer layer) {
  if (index > doc.layers.size()) {
    return layerOpFail(
        "add layer refused: index " + std::to_string(index) + " is past the end of a " +
        std::to_string(doc.layers.size()) + "-layer document. An insert at exactly " +
  ...
  return layerOpSucceed(name.empty() ? "add layer" : "add layer \"" + name + "\"", index);
}

LayerOpResult removeLayer(Document& doc, size_t index) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "remove layer", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "remove layer", kLockedLayerFrozen, &refusal)) return refusal;
  const std::string label = "remove " + layerOpDescribe(doc, index);
...
LayerOpResult duplicateLayer(Document& doc, size_t index) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "duplicate layer", &refusal)) return refusal;
  // Deliberately no lock check -- duplicating reads the source and changes
  // nothing about it. See core/LayerOps.hpp's lock section.
  const std::string label = "duplicate " + layerOpDescribe(doc, index);
  ...
  doc.layers.insert(doc.layers.begin() + static_cast<std::ptrdiff_t>(index) + 1,
                    std::move(copy));
  return layerOpSucceed(label, index + 1);
}

LayerOpResult setLayerVisible(Document& doc, size_t index, bool visible) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "set layer visibility", &refusal)) return refusal;
  // No lock check: hiding a locked layer is allowed. See the header.
  doc.layers[index].visible = visible;
  return layerOpSucceed(std::string(visible ? "show " : "hide ") + layerOpDescribe(doc, index),
                        index);
}

LayerOpResult setLayerOpacity(Document& doc, size_t index, float opacity) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "set layer opacity", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "set layer opacity", kLockedLayerFrozen, &refusal))
    return refusal;
...
LayerOpResult setLayerBlend(Document& doc, size_t index, BlendMode mode) {
  ...
  if (!layerOpNotLocked(doc, index, "set layer blend mode", kLockedLayerFrozen, &refusal))
    return refusal;
  if (!blendModeAvailableForLayer(doc, index, mode)) {
    ...
    // second message.
    return layerOpFail(
        "set layer blend mode refused: \"" + std::string(blendModeName(mode)) +
        "\" is not available on " + layerOpDescribe(doc, index) +
  ...
  const std::string label =
      "set " + layerOpDescribe(doc, index) + " to blend mode \"" + blendModeName(mode) + "\"";
...
LayerOpResult addLayerMask(Document& doc, size_t index) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "add layer mask", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "add layer mask", kLockedLayerFrozen, &refusal))
    return refusal;
  if (doc.layers[index].mask.has_value()) {
    return layerOpFail(
        "add layer mask refused: " + layerOpDescribe(doc, index) +
  ...
  return layerOpSucceed("add mask to " + layerOpDescribe(doc, index), index);
}

LayerOpResult removeLayerMask(Document& doc, size_t index) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "remove layer mask", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "remove layer mask", kLockedLayerFrozen, &refusal))
    return refusal;
  if (!doc.layers[index].mask.has_value()) {
    return layerOpFail(
        "remove layer mask refused: " + layerOpDescribe(doc, index) +
...
LayerOpResult setLayerClipped(Document& doc, size_t index, bool clipped) {
  ...
  if (!layerOpNotLocked(doc, index, "set layer clipping", kLockedLayerFrozen, &refusal))
    return refusal;
  ...
  if (clipped) {
    if (index == 0) {
      return layerOpFail(
          "set layer clipping refused: " + layerOpDescribe(doc, index) +
    ...
    if (base == 0) {
      return layerOpFail(
          "set layer clipping refused: every layer beneath " + layerOpDescribe(doc, index) +
    ...
    if (!holdsPixels) {
      return layerOpFail(
          "set layer clipping refused: the nearest layer beneath " + layerOpDescribe(doc, index) +
          " that is not itself clipped is " + layerOpDescribe(doc, base - 1) + ", a " +
          std::string(layerKindName(below.kind)) +
  ...
  const std::string label =
      std::string(clipped ? "clip " : "unclip ") + layerOpDescribe(doc, index);
...
LayerOpResult setLayerName(Document& doc, size_t index, std::string name) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "rename layer", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "rename layer", kLockedLayerFrozen, &refusal)) return refusal;
  const std::string label = "rename " + layerOpDescribe(doc, index) + " to \"" + name + "\"";
...
LayerOpResult setLayerColorLabel(Document& doc, size_t index, std::string colorLabel) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "set layer colour label", &refusal)) return refusal;
  const std::string label =
...
LayerOpResult setLayerLinkGroup(Document& doc, size_t index, uint64_t group) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "set layer link", &refusal)) return refusal;
  const std::string label = (group == 0 ? "unlink " : "link ") + layerOpDescribe(doc, index);
...
LayerOpResult addLayerOp(Document& doc, size_t index, Op op) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "add op", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "add op", kLockedLayerFrozen, &refusal)) return refusal;
  const std::string label = "add " + opDisplayName(op) + " to " + layerOpDescribe(doc, index);
...
LayerOpResult removeLayerOp(Document& doc, size_t index, size_t opIndex) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "delete op", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "delete op", kLockedLayerFrozen, &refusal)) return refusal;
  ...
  const std::string label = "delete " + opDisplayName(doc.layers[index].ops.at(opIndex)) +
                            " from " + layerOpDescribe(doc, index);
...
LayerOpResult moveLayerOp(Document& doc, size_t index, size_t from, size_t to) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "move op", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "move op", kLockedLayerFrozen, &refusal)) return refusal;
  ...
  const std::string label = "move " + opDisplayName(doc.layers[index].ops.at(from)) +
                            " to position " + std::to_string(to) + " on " +
                            layerOpDescribe(doc, index);
...
LayerOpResult setLayerOp(Document& doc, size_t index, size_t opIndex, Op op) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "edit op", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "edit op", kLockedLayerFrozen, &refusal)) return refusal;
  if (!opInRange(doc, index, opIndex, "edit op", &refusal)) return refusal;
  const std::string label = "edit " + opDisplayName(op) + " on " + layerOpDescribe(doc, index);
...
LayerOpResult setLayerOpEnabled(Document& doc, size_t index, size_t opIndex, bool enabled) {
  LayerOpResult refusal;
  if (!layerOpInRange(doc, index, "enable op", &refusal)) return refusal;
  if (!layerOpNotLocked(doc, index, "enable op", kLockedLayerFrozen, &refusal)) return refusal;
