...
// `ctlSlider()` and `drawCurveWidget()` are `ui/MacPaintUI.cpp`'s own --
// defined there (still, and still used by everything else in that file that
// was already using them: the BRUSH panel's other sliders, the GRADE
// section's curve op editor), given external linkage there specifically so
// this second translation unit can reach them. Declared again here rather
// than through a shared header because nothing else needs them outside those
// two files, and a header with one caller is a header this codebase's own
