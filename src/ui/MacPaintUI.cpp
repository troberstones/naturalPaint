...
void drawUI(AppState& st, std::unique_ptr<PaintSim>& sim, GpuContext& gpu,
  ...
  if (ImGui::Begin("##controls", nullptr,
    ...
    //
    // ui/LabelledControl.cpp owns the state this reads (`g_labelColumn` and
    // friends) as file-static now, so this is the one call into that module
    // from outside its own six functions -- see its own comment.
    reportLabelColumnIfChanged(kControlsW, ImGui::GetContentRegionAvail().x);
  }
  ImGui::End();
