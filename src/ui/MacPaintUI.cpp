...
//
// **External linkage from here through the end of `ctlSliderInt()` below,
// deliberately breaking out of this file's anonymous namespace for those five
// names alone.** `ui/DynamicsMatrixPanel.cpp`'s `drawLinkEditor()` -- moved out
// of this file entire, ui/DynamicsMatrixPanel.hpp's own header explains why --
// calls `ctlSlider()` for its RANGE sliders, and an anonymous-namespace symbol
// is invisible outside the translation unit that defines it: internal linkage
// is a per-TU property, not a per-scope one a `using namespace` from a second
// file could see around. Reopened immediately after `ctlSliderInt()`, so
// everything else in this section (ctlCombo and the rest) keeps the internal
// linkage it always had.
}  // namespace
