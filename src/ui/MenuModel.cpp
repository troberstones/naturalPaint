...
const MenuItemSpec* specTable() {
  ...
  static const bool built = [] {
    ...
    // P2 "filters" track (PRD D11). No key equivalent, same reason as the
    // three above.
    ...
    // P2 "filters" track (PRD D12). No key equivalent: docs/shortcuts.md
    // assigns none.
...
bool menuActionEndsTransform(MenuAction action) noexcept {
  switch (action) {
    ...
    // P2 "filters" track: each rewrites the active layer's own texels or
...
MenuEffect menuActionEffect(MenuAction action) noexcept {
  switch (action) {
    ...
    // P2 "filters" track: each opens a modal (ui/DustScratchesDialog.hpp,
...
std::vector<MenuNode> buildMenuModel(const MenuContext& ctx) {
  ...
  // is left out of the tree rather than added disabled.
  {
    ...
    // Filter menu's seven items make the same trade.
    {
      ...
      // P2 "filters" track (PRD D12): Photoshop's own placement, appended
  ...
  // rather than beside Gaussian Blur, which is a different engine entirely.
  {
    ...
    // P2 "filters" track (PRD D11): the median-gated despeckle.
